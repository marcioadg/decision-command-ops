...
export const DecisionModalHeader = ({ decision, editMode, onEditToggle, onClose }: DecisionModalHeaderProps) => {
  ...
  const hasReflection = decision.reflection?.thirtyDay || decision.reflection?.questions?.length;
  const isReflectionComplete = decision.reflection?.thirtyDay?.answers?.length;
